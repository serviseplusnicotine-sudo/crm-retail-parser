...
// Проксі, з яким запущено ПОТОЧНИЙ інстанс браузера (для page.authenticate
// у withPuppeteerPage нижче) — окремо від пулу, бо весь браузер прив'язаний
// до одного проксі на весь час свого життя (див. коментар вище).
...
export async function rawFetch(url, opts = {}) {
        ...
        // Випадковий проксі з пулу на КОЖЕН запит — якщо один IP забанений
  // конкретним сайтом, це не "залипає" назавжди для всіх наступних запитів.
  const picked = pickRandomProxy();
        try {
