...
async function launchBrowser() {
  ...
  const browser = await withTimeout(
              puppeteer.default.launch({ headless: true, args: PUPPETEER_LAUNCH_ARGS }),
              ...
              try {
                            for (const query of puppeteerQueries) {
                                            const result = await withPuppeteerPageLimited(async (page) => {
                                                              ...
                                                              // ТИМЧАСОВЕ діагностичне логування (14.07) — поки з'ясовуємо,
                                                                                                    // чому iStore/МТА досі повертають "no-product" навіть після
                                                                                                    // фіксу URL пошуку й порогу довжини контейнера ціни. Прибрати,
                                                                                                    // коли причина знайдена й підтверджена.
                                                                                                    console.log(`[retail-parser][debug] ${config.name} query="${query}" candidates=${candidates.length}${candidates.length ? ' top=' + JSON.stringify(candidates.slice(0, 5).map(c => ({ t: c.title, p: c.price }))) : ''}`);
