...
//
// ПІДСУМОК (14.07, після перевірки): stealth-плагін НЕ дав жодної різниці
// (ідентичні cfCleared/candidates/textLen до і після) — див.
// fetchViaFlareSolverr нижче, куди перенесено основну надію на обхід.
// Лишаємо stealth увімкненим — не заважає, і теоретично може допомогти
// іншим сайтам у майбутньому.
...
// ---------- FlareSolverr (14.07, четверта діагностична спроба) ----------
// Окремий Render-сервіс (НЕ частина цього процесу) на базі headless
// Chrome, спеціально заточений під обхід Cloudflare/DDoS-Guard-подібних
// JS-перевірок. Причина, чому знадобився: розширене логування показало,
// що навіть Puppeteer зі stealth-плагіном (puppeteer-extra-plugin-stealth,
// див. getPuppeteerExtra нижче) не проходить Cloudflare Managed Challenge
// на mta.ua через наш резидентний проксі — cfCleared лишався false навіть
// після 20с очікування, pageTitle весь час "Just a moment...". FlareSolverr
// — окремий, широко використовуваний інструмент саме під цю задачу.
//
// ІЗОЛЯЦІЯ: розгорнутий як ОКРЕМИЙ Render-сервіс (srv-d9b1977avr4c73b7mba0,
// https://flaresolverr-aurj.onrender.com) — щоб знову не повторити
// інцидент 13.07 (одна спільна залежність кладе весь парсер). Якщо
// FlareSolverr впаде/буде недоступний — постраждають лише магазини з
// config.useFlareSolverr:true (iStore/МТА), решта (Jabko/Yablyka/GRO/
// JustBuy) продовжать працювати як і раніше.
...
async function fetchViaFlareSolverr(url, opts = {}) {
      // FlareSolverr сам піднімає headless Chrome на СВОЄМУ боці (Render,
  // датацентр-IP) — без проксі його запити так само потраплять під той
  // самий бан за IP датацентру, від якого ми й тікаємо. Передаємо той
  // самий резидентний проксі з нашого пулу через параметр proxy — FlareSolverr
  // проведе свій внутрішній Chrome через нього, поєднуючи "справжній" (не
  // датацентр) IP з розв'язанням самого JS-виклику.
  const picked = opts.useProxy ? pickRandomProxy() : null;
      const body = {
              cmd: 'request.get',
              url,
...
//   useFlareSolverr            — чи пробувати FlareSolverr (окремий сервіс, обхід
//                                Cloudflare/антибот) ПІСЛЯ швидкого fetch, але
//                                ПЕРЕД власним Puppeteer-фолбеком (iStore/МТА)
...
export async function scrapeStore(config, product) {
  ...
  // 1.5) FlareSolverr — спеціалізований обхід Cloudflare/антибот-перевірок
  // (14.07) — лише для магазинів з config.useFlareSolverr:true. Пробуємо
  // ПІСЛЯ звичайного дешевого fetch (крок 1, який і так working для
  // більшості магазинів), але ПЕРЕД важким власним Puppeteer-фолбеком —
  // FlareSolverr сам піднімає headless Chrome на своєму боці, тож наш
  // Puppeteer був би зайвим подвійним рендером, якщо FlareSolverr вже
  // впорався. Пробуємо лише 2 перших (найдовших) варіанти запиту —
  // FlareSolverr теж недешевий за часом (власний headless Chrome +
  // очікування Cloudflare-виклику), не сенс перебирати всі 4-6 варіантів.
  if (config.useFlareSolverr) {
          for (const query of queries.slice(0, 2)) {
                    try {
                                const url = config.searchUrl(query);
                                const html = await fetchViaFlareSolverr(url, { useProxy: !!config.useProxy });
                                const $ = cheerio.load(html);
                                const candidates = extractCandidatesCheerio($, url);
                                // ТИМЧАСОВЕ діагностичне логування (14.07, п'ята спроба) — перший
                      // прогін через FlareSolverr дав candidates=0 для ОБОХ магазинів
                      // (без явної помилки — сама відповідь прийшла успішно), а на
                      // другому запиті сам FlareSolverr впав з HTTP 502 (ймовірно,
                      // OOM/перевантаження на Render free/starter-тарифі — headless
                      // Chrome усередині FlareSolverr теж важкий). candidates=0 без
                      // помилки означає: або FlareSolverr отримав від сайту сторінку-
                      // блокування/капчу (не справжню видачу), або структура HTML не
                      // підійшла під generic-екстрактор. Додаємо title/len/uri сторінки,
                      // яку реально повернув FlareSolverr — щоб відрізнити ці випадки.
                      console.log(`[retail-parser][debug] ${config.name} (FlareSolverr) query="${query}" htmlLen=${html.length} htmlTitle="${($('title').text() || '').replace(/\s+/g, ' ').trim().slice(0, 120)}" anchors=${$('a[href]').length} candidates=${candidates.length}${candidates.length ? ' top=' + JSON.stringify(candidates.slice(0, 5).map(c => ({ t: c.title, p: c.price }))) : ''}`);
                                const best = pickBest(candidates, query);
                                if (best) {
                                              return {
                                                              store: config.name, price: best.price, available: best.available,
                                ...
                                break; // сам сервіс недоступний/впав — коротші запити цього не виправлять
                    }
          }
  }
