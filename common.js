...
// Проксі, з яким запущено ПОТОЧНИЙ інстанс браузера — див. коментар у
// launchBrowser() нижче: зараз завжди null, бо жоден активний споживач
// Puppeteer не потребує проксі.
...
async function launchBrowser() {
  ...
  // iStore/МТА — єдині, кому теоретично потрібен проксі — мають
  // usePuppeteerFallback:false і Puppeteer через launchBrowser() не
  // викликають узагалі. Тобто ЖОДЕН поточний споживач Puppeteer не
  // потребує проксі — тому браузер тепер ЗАВЖДИ запускається напряму.
  // Якщо колись iStore/МТА повернуть usePuppeteerFallback:true з робочим
  // (резидентним) проксі — сюди треба повертати вибір проксі, і краще
  // окремим виділеним інстансом браузера, а не спільним для всіх
  // магазинів (щоб падіння проксі знову не тягнуло за собою Jabko/Yablyka).
  currentBrowserProxyConfig = null;
...
export async function scrapeStore(config, product) {
  ...
  // usePuppeteerFallback: false (iStore/МТА, поки прокси-пул забанений
  // сайтом цілком) — якщо ще й fetch-крок впав (403 тощо), чесно
