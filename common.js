...
export async function scrapeStore(config, product) {
  ...
  if (config.useFlareSolverr) {
            for (const query of queries.slice(0, 2)) {
                        try {
                                      const url = config.searchUrl(query);
                                      // ДІАГНОСТИЧНИЙ ПЕРЕМИКАЧ (14.07, п'ята спроба, за рішенням Ігоря):
                          // попередній прогін показав підозріло ІДЕНТИЧНУ за розміром
                          // (~250КБ) відповідь FlareSolverr для ДВОХ різних доменів (iStore
                          // й МТА), з 0 посилань і "голим" доменом замість справжнього
                          // title сторінки — це більше схоже на сторінку помилки самого
                          // проксі, ніж на реальний контент сайту. FLARESOLVERR_NO_PROXY=true
                          // (env var на Render, тимчасово) прибирає резидентний проксі ЛИШЕ
                          // з виклику FlareSolverr — щоб перевірити, чи проблема саме в
                          // проксі-пулі, чи в FlareSolverr/самих сайтах. Puppeteer-фолбек
                          // нижче й далі використовує проксі як завжди (config.useProxy) —
                          // цей перемикач впливає ТІЛЬКИ на крок 1.5.
